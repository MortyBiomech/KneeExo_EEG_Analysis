...
%
% Two things to state in the Methods
% ---------------------------------
% The p-value is (#null >= observed + 1)/(NPERM + 1), so with NPERM = 5000
% the smallest attainable value is 1/5001 = 2.0e-4. A cluster reported at
% that value has hit the floor: write it as p < 0.001 and give the
% permutation count, never as a smaller exact number.
%
% NPERM is far below the number of distinct relabellings. With k conditions
% and n participants there are (k!)^n of them, so 3 conditions and 14
% participants gives 6^14, about 7.8e10. The test is therefore a Monte Carlo
% approximation, and its own sampling error is roughly sqrt(p(1-p)/NPERM):
% about 0.003 at p = 0.05. A p that sits within a few of those of your
% decision threshold should not be treated as decisive on its own.
%
% The only change from the RESULTS_BEHAVIOUR.m version is the completeness
% check below, which warns and changes no number.

    Y = permute(C, [1 3 2]);                 % nSubj x nCond x nSamp
    [n, k, ns] = size(Y);

    % The sums of squares below partition the total only for complete,
    % balanced data. 'omitnan' in the means keeps a missing value from
    % poisoning everything, but it does not make the decomposition valid, and
    % a large enough gap can drive ssErr negative. This warns rather than
    % silently returning a meaningless F.
    if anynan(Y)
        warning('cluster_perm_1d:MissingData', ...
            ['%d of %d values are NaN. The repeated-measures sums of ' ...
             'squares assume complete data, so the F values below are not ' ...
             'trustworthy.'], nnz(isnan(Y)), numel(Y));
    end
