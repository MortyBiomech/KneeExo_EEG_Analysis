...
txt{end+1} = ['  within-participant SD. The Low to High rise in the rating it ' ...
    'is set against is'];
txt{end+1} = ['  NOT computed here. Take it from RUN_RESULTS_BEHAVIOUR and ' ...
    'check it before the'];
txt{end+1} = '  number goes into the manuscript.';
txt{end+1} = '';
...
txt{end+1} = '  because a bound in dB has no scale to set the rating rise against.';
txt{end+1} = '';
...
if isfield(S, 'randSlopeConverged')
    txt{end+1} = sprintf(['Random slope check: %d of %d cluster models ' ...
        'converged.'], S.randSlopeConverged, S.randSlopeTotal);
    txt{end+1} = '';
end

% Which specifications disagree with the primary about what survives. This is
% the guard that matters. A re-run that quietly changes which clusters clear
% the correction under some specification would otherwise leave the manuscript
% describing a result the code no longer produces.
if isfield(results, 'sensitivity') && ~isempty(results.sensitivity)
    txt = [txt, disagreement_lines(results, L, S)];
end
...
lines{end+1} = sprintf(['\\caption{\\textbf{Joint test of each cortical ' ...
    'cluster''s contribution to perceived difficulty.} The three band-power ' ...
    'features of one cluster, centred within participant, were added together ' ...
    'to the mediation model and tested jointly by a Wald test with ' ...
    'Satterthwaite denominator degrees of freedom. $n$ is trials, $k$ is ' ...
    'participants contributing a component to that cluster and a complete set ' ...
    'of covariates. $q$ values are Benjamini and Hochberg corrected across the ' ...
    '%d cluster tests, which are the whole family; the %d single features in ' ...
    'Table~\\ref{tab:lmm21feature} were not tested.}'], height(C), S.nFeatures);
lines{end+1} = '\label{tab:lmm21cluster}';
lines{end+1} = '\begin{tabular}{lrrlrr}';
lines{end+1} = '\toprule';
lines{end+1} = 'Cluster & $n$ & $k$ & $F$ & $p$ & $q$ \\';
lines{end+1} = '\midrule';

labels = containers.Map(L.clusters(:, 1), L.clusters(:, 3));

% The table follows the cluster order of LMM21_CONFIG, not the p value order
% of RESULTS.CLUSTER, so that left and right of a pair sit together and the
% reader is not invited to read the ranking as the finding.
order = string(L.clusters(:, 1));
Cs = table();
for k = 1:numel(order)
    Cs = [Cs; C(C.Cluster == order(k), :)]; %#ok<AGROW>
end
if height(Cs) ~= height(C)
...
for i = 1:height(Cs)
    ...
    if Cs.Converged(i)
        lines{end+1} = sprintf(['%s & %d & %d & $F(%d, %.0f) = %.2f$ & %.3f ' ...
            '& %.3f \\\\'], labels(nm), Cs.N(i), Cs.nSubjects(i), Cs.DF1(i), ...
            Cs.DF2(i), Cs.FStat(i), Cs.pValue(i), Cs.q(i)); %#ok<AGROW>
    else
        lines{end+1} = sprintf(['%s & %d & %d & \\multicolumn{3}{c}{did not ' ...
            'fit} \\\\'], labels(nm), Cs.N(i), Cs.nSubjects(i)); %#ok<AGROW>
    end
end
...
if isfield(S, 'randSlopeConverged')
    fprintf(['  Random slope models: %d of %d converged. The primary model uses a\n' ...
        '    by-participant random intercept because that is the random-effects\n' ...
        '    structure of the mediation model the cortical terms were added to,\n' ...
        '    NOT because the slope failed. If the two specifications disagree\n' ...
        '    about what survives, the Results have to say so.\n'], ...
...
% ----------------------------------------------------------------------------
function lines = disagreement_lines(results, L, S)
%DISAGREEMENT_LINES  Which specifications contradict the primary conclusion.
%
%  The primary specification decides what the manuscript claims. A
%  specification that disagrees with it does not overturn the claim, but it
%  does have to be reported, and the only way to guarantee that is to print it
%  here rather than leave it to be noticed in the CSV.

lines = {};
Sen = results.sensitivity;
if isempty(Sen)
    return
end

keys = unique(string(Sen.Spec), 'stable');
rows = {};
for k = 1:numel(keys)
    sub = Sen(string(Sen.Spec) == keys(k) & Sen.Converged, :);
    if isempty(sub)
        continue
    end
    surv = sub.Cluster(sub.q < L.stats.fdrQ);
    % The names go in as one char row, not as a cell. A struct built with an
    % empty cell value collapses to a 0x0 struct array, and a specification
    % where nothing survives is exactly the case that would trigger it.
    rows{end+1} = struct('key', keys(k), 'n', numel(surv), ...
        'names', strjoin(cellstr(string(surv)), ', '), ...
        'minQ', min(sub.q)); %#ok<AGROW>
end

lines{end+1} = 'Specification check';
lines{end+1} = repmat('-', 1, 74);
lines{end+1} = sprintf(['  primary      %d cluster(s) survive q < %.2f, ' ...
    'smallest q %.4f'], S.nSurvivingFDR, L.stats.fdrQ, S.minQ);
for k = 1:numel(rows)
    r = rows{k};
    if r.n == 0
        lines{end+1} = sprintf('  %-12s none survive, smallest q %.4f', ...
            r.key, r.minQ); %#ok<AGROW>
    else
        lines{end+1} = sprintf('  %-12s %d survive: %s', ...
            r.key, r.n, r.names); %#ok<AGROW>
    end
end
lines{end+1} = '';
...
differ = {};
for k = 1:numel(rows)
    if (rows{k}.n > 0) ~= (S.nSurvivingFDR > 0)
        differ{end+1} = char(rows{k}.key); %#ok<AGROW>
    end
end
if ~isempty(differ)
    lines{end+1} = sprintf(['  *** %s disagree(s) with the primary about ' ...
        'what survives.'], strjoin(differ, ', '));
    lines{end+1} = '      The Results have to report this, not only the table. ***';
    lines{end+1} = '';
end
