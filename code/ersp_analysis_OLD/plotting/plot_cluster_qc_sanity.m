function plot_cluster_qc_sanity(s, p, outputPath, studyName)
% PLOT_CLUSTER_QC_SANITY  Quick-look figures for one cluster's QC'd results.
%
%   PLOT_CLUSTER_QC_SANITY(S, P, OUTPUTPATH, STUDYNAME)
%
% Two figures:
...
% however many panels that figure has.
    ...
    L.panelTitleRowMM = 3;    % each panel's own title (e.g. 'Low Pressure')
    L.eventLabelRowMM = 5;    % rotated FlxS / FlxE-ExtS / ExtE labels,
...
% at p.plot.rasterDPI.
    for i = 1:numel(p.plot.formats)
        ...
        switch fmt
            ...
            otherwise
                warning('plot_cluster_qc_sanity:UnknownFormat', ...
