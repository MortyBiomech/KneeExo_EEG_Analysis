...
%% 1. Set up ------------------------------------------------------------
clc; clear;
% which(), not mfilename('fullpath'): running a %% section via the
% Editor's "Run Section" button can evaluate it through a temporary
% Live Editor helper file, in which case mfilename('fullpath') reports
% that temp file's location instead of this one. which() looks up this
% file by name instead, which is unaffected by how the section was run.
...
addpath(genpath(find_analysis_root(thisFile)));   % both compute/ and figures/
...
load(p.subjectsIcsFile, 'SUBJECTS_ICS');
...
for r = 1:numel(p.roiStudyFiles)
    ...
    clusterData = compute_cluster_ersp_qc(clusterLabel, SUBJECTS_ICS, icatimefPath, p);
...
function rootDir = find_analysis_root(thisFile)
% Walks up from THISFILE to the folder holding ersp_params.m.
%
% That file marks the analysis root and is shared by the compute and the
% figure side, so genpath from there puts BOTH trees plus the shared
% params on the path no matter how deep this script sits.
%
% Marker-based, not a fixed count of fileparts() calls. A hardcoded depth
% breaks the moment the tree is reorganised, which is exactly what
% happened when compute/ and figures/ were introduced.
    searchDir = fileparts(thisFile);
    while true
        if isfile(fullfile(searchDir, 'ersp_params.m'))
            rootDir = searchDir;
            return
        end
    ...
    error('find_analysis_root:NotFound', ...
        ['Could not find ersp_params.m above %s. It marks the analysis ' ...
         'root; this script cannot set up its path without it.'], thisFile);
end

function configDir = find_config_folder(thisFile)
% Walks up from THISFILE's own folder looking for a 'config' sibling, so
% this does not depend on how many folders deep this script sits.
    searchDir = fileparts(thisFile);
    while true
    ...
    error('find_config_folder:NotFound', ...
