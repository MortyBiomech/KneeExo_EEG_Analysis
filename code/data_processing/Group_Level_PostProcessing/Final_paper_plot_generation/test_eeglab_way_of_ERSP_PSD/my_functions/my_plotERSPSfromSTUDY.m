...
%         Personalized by Morteza Khosrotabar, TU Darmstadt 2025
% Created: 2021, last modified by Noelle 1/6/23, 
%                last modified by Morteza 14/10/25.
...
function ersp_results = my_plotERSPSfromSTUDY(STUDY, ALLEEG, savePath, ...
    myplotParams, clusters_to_plot, one_sub_per_cl, erspParamOverride, ...
    STUDY_title, ersp_results)
...
eventLabels = {'FlxS', 'ExtS', 'ExtE'}; % labels corresponding to evPlotLines
...
STUDY = pop_erspparams(STUDY, 'subbaseline', 'on', ...
    'timerange', [evPlotLines(1) evPlotLines(end)], 'freqrange',[3 130], ...
    'ersplim',[-1.5 1.5]);  
...
try
    for XX = 1:length(clusters_to_plot)
        ...
        switch mode
            case 'eeglab'
                tmpSTUDY = pop_statparams(STUDY, 'condstats', ...
                    condstats, 'method', statsMethod2, 'alpha', Alpha, ...
                    'mcorrect', mcorrect2, 'singletrials', singletrials);
            case  'fieldtrip'
        ...
        if erspParamOverride_flag == 1 % override parameters using myErspParams given in input
            ...
            [tmpSTUDY, allersp, alltimes, allfreqs, ...
                pgroup, all_pcond, pinter, events] = ...
                std_erspplot_myparams(tmpSTUDY, ALLEEG, myErspParams, ...
                'clusters', CL, 'logfreq', 'on', ...
    ...
    if plotStuff
        ...
        for groupi = 1:size(erspdata.mean,2)
            ...
            if ~isempty(refErspCond)
                ...
                savethisfig(gcf, strcat(figname,'.png'), ...
                    [savePath,'\ERSP\',myplotParams.figname,'\Cond_vs_Baseline\png'],'png')
                savethisfig(gcf, strcat(figname,'.fig'), ...
