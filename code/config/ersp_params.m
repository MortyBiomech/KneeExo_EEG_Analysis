...
%
% Paths are NOT here -- those come from kneeexo_config(). This file lives
% beside it in config/, so the same addpath finds both.
    %% Regions of interest -------------------------------------------------
...
% Passed to mod_std_precomp_v_forEEGlabv2021 as 'erspparams'. Morlet wavelets with a
...
% 'median latency baseline' is understood by mod_std_precomp_v_forEEGlabv2021 only:
...
% masks drawn on the difference plots.
%
% !!! THIS DOES NOT MATCH THE PUBLISHED FIGURES !!!
%
% In the original, Alpha was set to 0.05 at the top of the plotting
% function and then reassigned to 0.01 three separate times inside the
% cluster loop, with a comment warning that it was "hard coded in places".
% 0.01 is therefore the value that was in force for every figure in the
% paper, and 0.05 below is NOT it.
%
% build_stats_study passes this straight through as fieldtripalpha, so
% every *_ersp_qc_results.mat produced with the value below was computed at
% a different threshold than the published figures. Decide which one the
% manuscript reports, set it here, and regenerate. Whichever you pick, state
% it once in the Methods and never override it anywhere else.
    p.stats.alpha = 0.05;
...
% channel-level data. compute_cluster_ersp.m simply walks
...
% (the QC script + flag_bad_trials.m) exactly.
...
% Subject/IC assignment per cluster. Just the file NAME: run_ersp_stats
% resolves it under cfg.derived, because absolute paths do not belong in
% this file.
%
% Confirmed current for these 8 ROIs as of this run. If the clustering is
% ever redone, this has to be rebuilt too, and every result below it is
% stale until it is.
%
% The previous value was an absolute path ending in a separator, i.e. a
% folder rather than a file, so load() on it could not have worked.
    p.subjectsIcsFile = 'Subjects_ICs_in_clusters.mat';
    %% Plotting --------------------------------------------------------------
...
% data, and not plot_qc_sanity.m, which still shows the full range.
...
% pipeline derives from the .study filename (run_ersp_stats.m does
...
% lookup in compute_cluster_ersp.m and all existing .mat files keep
...
% Total figure HEIGHT, in mm, for PLOT_QC_SANITY's fully manual
...
% How far PLOT_QC_SANITY shifts the FlxS/ExtE event LABELS in
...
% text ('Cycle (%)' / 'Frequency (Hz)') in PLOT_QC_SANITY. Both
...
% draw_mid_event_line in PLOT_QC_SANITY). Gray rather than the
