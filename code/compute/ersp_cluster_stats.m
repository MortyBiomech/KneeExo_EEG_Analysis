...
%   PCOND        {1 x nGroups} condition mask. 1 means SIGNIFICANT: with a
%                numeric alpha set, std_stat_clusterpval returns FieldTrip's
%                stat.mask, which marks the pixels belonging to a cluster
%                that survived correction. (An earlier version of this
%                header said the opposite. Reading it the other way round is
%                what inverted two lines in the pre-cleanup plotting code;
%                see docs/ersp-precompute-provenance.md.)
%   PGROUP       {1 x nConditions} group mask, same polarity (empty for a
%                single group)
