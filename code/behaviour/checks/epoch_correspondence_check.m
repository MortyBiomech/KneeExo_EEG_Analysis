...
if isempty(thisFile) || contains(thisFile, 'LiveEditorEvaluationHelper')
    thisFile = which('epoch_correspondence_check');
end
if isempty(thisFile)
    error(['Cannot locate config/. Open epoch_correspondence_check.m and press Run, ' ...
...
for subID = subject_list
    ...
    % No cd. Loading into a struct keeps the working directory out of the
    % result and works whatever folder you launched from.
