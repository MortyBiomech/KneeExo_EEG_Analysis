...
%  recomputed on every styling change, and the cycle curves, which need the
%  per-subject master files. Both results ship in data/derived and are
%  loaded by default, so this script draws the whole figure with nothing
%  downloaded. See USE_PRECOMPUTED.
%
...
if isempty(thisFile) || contains(thisFile, 'LiveEditorEvaluationHelper')
    thisFile = which('run_results_behaviour');
end
if isempty(thisFile)
    error(['Cannot locate config/. Open run_results_behaviour.m and press Run, ' ...
...
fig_path = cfg.figures;     % <repo>/figures, NOT inside the data tree
...
% Precomputed curves and permutation results.
%
% Two different costs are avoided here. The cycle curves are built from the
% per-subject master files, which run to gigabytes and are not in the
% repository. The permutation tests take minutes and are recomputed on
% every styling change. Both results are small, a few hundred kB together,
% so they ship in data/derived and this script loads them by default.
%
% Set false to rebuild. That needs the per-subject masters present, and it
% is required after any change to the screening, the warp grid or the
% permutation settings. The load path checks those settings and refuses a
% file that was built with different ones.
USE_PRECOMPUTED  = true;
PRECOMPUTED_FILE = fullfile(cfg.derived, 'figure2_precomputed.mat');
...
% Settings the precomputed file must match. If any of these changed, a
% stored curve describes a different analysis and must not be reused.
precompSettings = struct('policy', POLICY, 'nsampEMG', NSAMP_EMG, ...
    'nsampTRK', NSAMP_TRK, 'flexFrac', FLEX_FRAC_PLOT, ...
    'nPerm', N_PERM, 'clustAlpha', CLUST_ALPHA, 'rngSeed', RNG_SEED);

buildFromMasters = ~USE_PRECOMPUTED || ~exist(PRECOMPUTED_FILE, 'file');

if ~buildFromMasters
    P = load(PRECOMPUTED_FILE);
    mismatch = settings_differ(P.settings, precompSettings);
    if isempty(mismatch)
        emgCurves = P.emgCurves;  emgSubs = P.emgSubs;
        trkCurves = P.trkCurves;  trkSubs = P.trkSubs;
        emgClust  = P.emgClust;   emgEta  = P.emgEta;
        trkClust  = P.trkClust;   trkEta  = P.trkEta;
        fprintf('\nCurves and permutation results loaded from\n  %s\n', ...
            PRECOMPUTED_FILE);
        if isfield(P, 'stamp')
            fprintf('  built %s by %s\n', P.stamp.written, P.stamp.builder);
        end
    else
        buildFromMasters = true;
        warning('run_results_behaviour:StalePrecomputed', ...
            ['%s was built with different settings (%s), so it is being ' ...
             'rebuilt. This needs the per-subject master files.'], ...
            PRECOMPUTED_FILE, strjoin(mismatch, ', '));
    end
end

if buildFromMasters
    fprintf('\nBuilding cycle curves from the per-subject masters ...\n');
...
if buildFromMasters
    ...
    % Written to data/derived, not to the data tree: this is one of the
    % files the repository ships so that Figure 2 needs no download. Commit
    % it whenever you rebuild it.
    settings = precompSettings; %#ok<NASGU>
    stamp = build_stamp(cfg, struct('script', 'run_results_behaviour')); %#ok<NASGU>
    if ~exist(cfg.derived, 'dir'), mkdir(cfg.derived); end
    save(PRECOMPUTED_FILE, 'emgCurves', 'emgSubs', 'trkCurves', 'trkSubs', ...
        'emgClust', 'emgEta', 'trkClust', 'trkEta', 'settings', 'stamp');
    fprintf('\nPrecomputed curves and clusters written to\n  %s\n', ...
        PRECOMPUTED_FILE);
end
...
%% 13. Export
% ---------------------------------------------------------------------
exportgraphics(fig, fullfile(fig_path,'figure2.pdf'), ...
    'ContentType','vector','BackgroundColor','white');
exportgraphics(fig, fullfile(fig_path,'figure2.png'), ...
...
function names = settings_differ(stored, current)
% Field names on which two settings structs disagree. Empty when they match.
% A precomputed curve built under a different warp grid or screening policy
% describes a different analysis, and reusing it would be silent.

    names = {};
    f = fieldnames(current);
    for k = 1:numel(f)
        if ~isfield(stored, f{k}) || ~isequal(stored.(f{k}), current.(f{k}))
