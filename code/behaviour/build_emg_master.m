function [masterIndex, masterTrials] = build_emg_master(cfg, subject_list)
% BUILD_EMG_MASTER  One row per raw EMG epoch, with flags instead of deletions.
%
%   [masterIndex, masterTrials] = build_emg_master()
%   [masterIndex, masterTrials] = build_emg_master(cfg)
%   [masterIndex, masterTrials] = build_emg_master(cfg, subject_list)
%
% THE PRINCIPLE. Every raw trial and every raw epoch gets a row, and no row
% is ever removed. Quality decisions are recorded as boolean columns and
% applied at analysis time, by screen_epochs. Position never carries
% identity: each row is identified by (SubjectID, RawTrial, RawEpoch),
% assigned before anything is judged good or bad.
%
% WHY. The previous pipeline compacted at two levels. Trials without EMG
% advanced a separate counter jj, so Main_data sat at compacted positions
% while its metadata was read at raw ones, and the behaviour table inherited
% compacted trial numbers while the tracking pipeline kept raw ones. Within a
% trial, epochs under 2000 samples were skipped with a counter kk, and the
% builder then read the signal at the raw index k but the event boundaries at
% kk, so later epochs in that trial were warped on another epoch's reversal
% point. Neither fault announces itself: the values stay valid, they simply
% attach to the wrong row. Keeping every row and flagging makes both
% impossible.
%
% SUBJECT LIST. Defaults to cfg.subjects, which is 5 to 18 and INCLUDES
% sub-10. That is deliberate. Sub-10's EMG stream exists but contains only
% zeros, and this builder records that as SignalHasVariance = false rather
% than omitting the participant. cfg.subjectsEMG, which drops sub-10, is for
% analyses, not for this.
%
...
%                   Sig      cell array of 4 x nSamples signals, indexed by
%                            row of E, empty where absent
%                   Tim      per-epoch timestamps
%                   muscles  channel order
%   overall     : emg_master_index.mat, the concatenated tables without
%                 signals, for querying and for building the behaviour table.
%
% Nothing here screens, normalises or warps. Those are analysis steps and
% belong downstream, where the flags are applied.
%
% See also BUILD_TRACKING_MASTER, SCREEN_EPOCHS, BUILD_BEHAVIOUR_TABLE.
    ...
    if isempty(cfg.raw)
        error('build_emg_master:NoRawPath', ...
    ...
    for subID = subject_list
        ...
        % No cd. The original changed directory to load, which breaks as soon
        % as anything else is running and makes the working directory part of
        % the result.
        ...
        % and a real-time window that can be matched against the EXP stream if
        % the event translation ever needs verifying independently.
        ...
        for j = 1:nTrials
            ...
            sp = {};
            if isfield(ep, 'EMG_stream') && ...
               isfield(ep.EMG_stream, 'Sensors_Preprocessed')
            ...
            for k = 1:nEpPerTrial(j)
                ...
                if hasEMG && numel(sp) >= k && ~isempty(sp{1, k}) && allFound
                    ...
                    if isfield(ep.EMG_stream, 'Times') && ...
                       numel(ep.EMG_stream.Times) >= k
        ...
        Tr = trial_table(Trials_Info, Epochs_FlextoFlex_based, subID, nEpPerTrial);
    ...
    report_emg_master(masterIndex, masterTrials);

end


% ------------------------------------------------------------------------
function Tr = trial_table(Trials_Info, Epochs, subID, nEpPerTrial)
...
% ------------------------------------------------------------------------
function report_emg_master(masterIndex, masterTrials)
...
% dropped; this is what a downstream screen would exclude.
    ...
    usable = masterIndex.IsExperiment & masterIndex.EpochHasSignal & ...
             masterIndex.SignalHasVariance & masterIndex.EpochLongEnough & ...
             masterIndex.EventsValid & masterIndex.AllMusclesFound;
    ...
    if ~isempty(noEp)
        disp(noEp(:, {'SubjectID','RawTrial','Pressure','Score', ...
                      'Description','TrialHasEMG'}));
    end
    ...
    fprintf(['\nNo rows were removed. Apply the flags at analysis time, ' ...
             'through screen_epochs, and report the counts above in the ' ...
             'Methods so each criterion''s effect is visible rather than ' ...
             'implicit.\n']);
